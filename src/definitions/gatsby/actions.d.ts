...
interface Job {
  [key: string]: unknown; // job context
...
interface CreatePage {
  ...
  context?: Record<string, unknown>; // the context data which will be passed to the component as props.pathContext (also as the context argument for the graphql query)
}
...
interface CreateNodeInternal {
  ...
  type: string; // an arbitrary globally unique type chosen by the plugin creating the node.
...
export interface Actions {
  ...
  setPluginStatus(status: Record<string, unknown>, plugin: Plugin): void;
